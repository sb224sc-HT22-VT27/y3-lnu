# Function to compute even parity for each row in the matrix
def compute_even_parity(matrix):
    row_parity_results = []
    ...
    for row in matrix:
        ...
        parity = 0 if count_of_ones % 2 == 0 else 1
...
# Function to compute even parity for each column in the matrix
...
# Compute even parity for rows
row_parity_results = compute_even_parity(matrix)

# Compute even parity for columns
column_parity_results = compute_column_parity(matrix)

# Output the results
print("Row Parity:")
for row, parity in zip(matrix, row_parity_results):
    print(f"Row: {row}, Even Parity: {parity}")

print("\nColumn Parity:")
for i, parity in enumerate(column_parity_results):
    print(f"Column {i+1}: Even Parity: {parity}")
